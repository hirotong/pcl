...
__global__ void
colorKernel(const PtrStepSz<unsigned char> labels, PtrStep<uchar4> output)
{
...
__global__ void
mixedColorKernel(const PtrStepSz<unsigned char> labels,
                 PtrStepSz<uchar4> rgba,
                 PtrStep<uchar4> output)
{
  int x = threadIdx.x + blockIdx.x * blockDim.x;
  int y = threadIdx.y + blockIdx.y * blockDim.y;
...
void
pcl::device::colorLMap(const Labels& labels,
...
void
pcl::device::mixedColorMap(const Labels& labels,
                           const DeviceArray<uchar4>& map,
                           const Image& rgba,
                           Image& output)
{
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
/// TODO implement getError string for NPP and move this to the same place with
/// cudaSafeCall
...
void
___nppSafeCall(int err_code, const char* file, const int line, const char* func = "")
{
  if (err_code < 0) {
    char buf[4096];
...
void
pcl::device::Dilatation::prepareRect5x5Kernel(DeviceArray<unsigned char>& kernel)
{
...
void
pcl::device::Dilatation::invoke(const Mask& src, const Kernel& kernel, Mask& dst)
{
  ...
  nppSafeCall(nppiDilate_8u_C1R(src.ptr(ANCH_Y) + ANCH_X,
                                (int)src.step(),
                                dst.ptr(ANCH_Y) + ANCH_X,
                                (int)dst.step(),
                                sz,
                                kernel,
                                ksz,
...
namespace device {
__global__ void
fgDepthKernel(const PtrStepSz<unsigned short> depth1,
              const PtrStep<unsigned char> inv_mask,
              PtrStep<unsigned short> depth2)
{
  ...
  if (x < depth1.cols && y < depth1.rows) {
    ...
    depth2.ptr(y)[x] =
        inv_mask.ptr(y)[x] ? d : std::numeric_limits<unsigned short>::max();
  }
}
} // namespace device
} // namespace pcl
...
void
pcl::device::prepareForeGroundDepth(const Depth& depth1,
                                    Mask& inverse_mask,
                                    Depth& depth2)
{
...
float
pcl::device::computeHue(int rgba)
{
...
namespace device {
__global__ void
computeHueKernel(const PtrStepSz<int> rgba,
                 const PtrStep<unsigned short> depth,
                 PtrStep<float> hue)
{
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
...
void
pcl::device::computeHueWithNans(const Image& rgba, const Depth& depth, HueImage& hue)
{
...
namespace device {
__global__ void
reprojectDepthKenrel(const PtrStepSz<unsigned short> depth,
                     const Intr intr,
                     PtrStep<float4> cloud)
{
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
...
void
pcl::device::computeCloud(const Depth& depth, const Intr& intr, Cloud& cloud)
{
