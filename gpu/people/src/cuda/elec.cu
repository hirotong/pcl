...
void
pcl::device::ConnectedComponents::initEdges(int rows,
                                            int cols,
...
__global__ void
computeEdgesKernel(const PtrStepSz<unsigned char> labels,
                   const PtrStep<unsigned short> depth,
                   const Intr intr,
                   const int num_parts,
                   const float sq_radius,
                   PtrStep<Edges> output)
{
  int x = threadIdx.x + blockIdx.x * blockDim.x;
  int y = threadIdx.y + blockIdx.y * blockDim.y;
...
void
pcl::device::ConnectedComponents::computeEdges(const Labels& labels,
                                               const Depth& depth,
                                               int num_parts,
                                               float sq_radius,
                                               DeviceArray2D<unsigned char>& edges)
...
#pragma unroll
      for (int j = 0; j < TPB_X; ++j) {
        ...
        // if there is a neighboring element with a smaller label, update the
        // equivalence tree of the processed element
        //(the tree is always flattened in this stage so there is no need to use
...
findRoot(const PtrStepSz<int>& comps, int label)
{
  for (;;) {
...
__global__ void
mergeKernel(const TilesMerge tm)
{
  tm();
}
} // namespace device
...
namespace device {
__global__ void
flattenTreesKernel(PtrStepSz<int> comps, const PtrStep<Edges> edges)
{
  int x = threadIdx.x + blockIdx.x * blockDim.x;
  int y = threadIdx.y + blockIdx.y * blockDim.y;
...
pcl::device::ConnectedComponents::labelComponents(
    const DeviceArray2D<unsigned char>& edges, DeviceArray2D<int>& comps)
{
