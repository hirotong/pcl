...
void
pcl_reconstruct(Cloud_t::Ptr cloud, std::string name)
{
...
main(int argc, char* argv[])
{
  ...
  desc.add_options()("help", "Show the help message")(
      ...
      "ply_saved_name",
      po::value<std::string>(),
      "the name with full path hwere you want to save the output mesh")
  ...
  po::variables_map vm;
  po::store(
      po::command_line_parser(argc, argv).options(desc).positional(pos_args).run(), vm);
  po::notify(vm);
